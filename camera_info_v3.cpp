...
// Helper to dynamically replace usb/vid/pid with pci/ven/dev while preserving the instance ID
void SpoofDevicePathW(wchar_t* path, size_t maxLen) {
    if (!path) return;
    
    wchar_t temp[1024];
    wcsncpy(temp, path, 1023);
    temp[1023] = 0;
    
    wchar_t* p = wcsstr(temp, L"usb#");
    if (p) { wcsncpy(p, L"pci#", 4); }
    else { p = wcsstr(temp, L"USB#"); if (p) wcsncpy(p, L"PCI#", 4); }
    
    p = wcsstr(temp, L"vid_");
    if (p) { wcsncpy(p, L"ven_", 4); }
    else { p = wcsstr(temp, L"VID_"); if (p) wcsncpy(p, L"VEN_", 4); }
    
    p = wcsstr(temp, L"pid_");
    if (p) { wcsncpy(p, L"dev_", 4); }
    else { p = wcsstr(temp, L"PID_"); if (p) wcsncpy(p, L"DEV_", 4); }
    
    wcsncpy(path, temp, maxLen - 1);
    path[maxLen - 1] = 0;
}

void SpoofDevicePathA(char* path, size_t maxLen) {
    if (!path) return;
    char temp[1024];
    strncpy(temp, path, 1023);
    temp[1023] = 0;
    
    char* p = strstr(temp, "usb#");
    if (p) { strncpy(p, "pci#", 4); }
    else { p = strstr(temp, "USB#"); if (p) strncpy(p, "PCI#", 4); }
    
    p = strstr(temp, "vid_");
    if (p) { strncpy(p, "ven_", 4); }
    else { p = strstr(temp, "VID_"); if (p) strncpy(p, "VEN_", 4); }
    
    p = strstr(temp, "pid_");
    if (p) { strncpy(p, "dev_", 4); }
    else { p = strstr(temp, "PID_"); if (p) strncpy(p, "DEV_", 4); }
...
LSTATUS WINAPI Hook_RegQueryValueExW(HKEY hKey, LPCWSTR lpValueName, LPDWORD lpReserved, LPDWORD lpType, LPBYTE lpData, LPDWORD lpcbData) {
    if (lpValueName && (_wcsicmp(lpValueName, L"FriendlyName") == 0 || _wcsicmp(lpValueName, L"DeviceDesc") == 0)) {
        if (!lpData) {
            LSTATUS status = orig_RegQueryValueExW(hKey, lpValueName, lpReserved, lpType, lpData, lpcbData);
            if (lpcbData && *lpcbData < 512) *lpcbData = 512; // Force larger buffer
            return status;
        }
    }
    if (lpValueName && (_wcsicmp(lpValueName, L"DevicePath") == 0 || _wcsicmp(lpValueName, L"SymbolicLink") == 0)) {
        if (!lpData) {
            LSTATUS status = orig_RegQueryValueExW(hKey, lpValueName, lpReserved, lpType, lpData, lpcbData);
            if (lpcbData && *lpcbData < 1024) *lpcbData = 1024; // Force larger buffer
            return status;
        }
    }
    ...
    if (status == ERROR_SUCCESS && lpValueName && lpData && lpcbData) {
        if (_wcsicmp(lpValueName, L"FriendlyName") == 0 || _wcsicmp(lpValueName, L"DeviceDesc") == 0) {
            ...
            SpoofDevicePathW((wchar_t*)lpData, *lpcbData / sizeof(wchar_t));
            LogA("[HOOK] RegQueryValueExW spoofed %ws", lpValueName);
        }
    }
    return status;
...
LSTATUS WINAPI Hook_RegQueryValueExA(HKEY hKey, LPCSTR lpValueName, LPDWORD lpReserved, LPDWORD lpType, LPBYTE lpData, LPDWORD lpcbData) {
    if (lpValueName && (_stricmp(lpValueName, "FriendlyName") == 0 || _stricmp(lpValueName, "DeviceDesc") == 0)) {
        if (!lpData) {
            LSTATUS status = orig_RegQueryValueExA(hKey, lpValueName, lpReserved, lpType, lpData, lpcbData);
            if (lpcbData && *lpcbData < 512) *lpcbData = 512;
            return status;
        }
    }
    if (lpValueName && (_stricmp(lpValueName, "DevicePath") == 0 || _stricmp(lpValueName, "SymbolicLink") == 0)) {
        if (!lpData) {
            LSTATUS status = orig_RegQueryValueExA(hKey, lpValueName, lpReserved, lpType, lpData, lpcbData);
            if (lpcbData && *lpcbData < 1024) *lpcbData = 1024;
            return status;
        }
    }
    ...
    if (status == ERROR_SUCCESS && lpValueName && lpData && lpcbData) {
        if (_stricmp(lpValueName, "FriendlyName") == 0 || _stricmp(lpValueName, "DeviceDesc") == 0) {
            ...
            SpoofDevicePathA((char*)lpData, *lpcbData);
            LogA("[HOOK] RegQueryValueExA spoofed %s", lpValueName);
        }
    }
    return status;
...
BOOL WINAPI Hook_SetupDiGetDeviceRegistryPropertyW(HDEVINFO a1, PSP_DEVINFO_DATA a2, DWORD prop, PDWORD a4, PBYTE buf, DWORD size, PDWORD reqSize) {
    if (!buf || size == 0) {
        ...
        if (reqSize && (prop == SPDRP_FRIENDLYNAME || prop == SPDRP_DEVICEDESC)) {
            if (*reqSize < 512) *reqSize = 512;
        }
        if (reqSize && prop == SPDRP_HARDWAREID) {
    ...
    if (res && buf && size > 0) {
        if (prop == SPDRP_REMOVAL_POLICY && size >= sizeof(DWORD)) {
            ...
            // Need to double null terminate hardware IDs.
            // Let's dynamically spoof it by replacing USB\VID with PCI\VEN in the first string.
            SpoofDevicePathW((wchar_t*)buf, size / sizeof(wchar_t));
            // Ensure double null termination just in case
            buf[size - 1] = 0;
            buf[size - 2] = 0;
...
BOOL WINAPI Hook_SetupDiGetDeviceRegistryPropertyA(HDEVINFO a1, PSP_DEVINFO_DATA a2, DWORD prop, PDWORD a4, PBYTE buf, DWORD size, PDWORD reqSize) {
    ...
    if (res && buf && size > 0) {
        if (prop == SPDRP_REMOVAL_POLICY && size >= sizeof(DWORD)) {
            ...
            SpoofDevicePathA((char*)buf, size);
            buf[size - 1] = 0;
            buf[size - 2] = 0;
...
BOOL WINAPI Hook_SetupDiGetDevicePropertyW(HDEVINFO a1, PSP_DEVINFO_DATA a2, const DEVPROPKEY* propKey, DEVPROPTYPE* propType, PBYTE buf, DWORD size, PDWORD reqSize, DWORD flags) {
    if (propKey && (!buf || size == 0)) {
        BOOL res = orig_SetupDiGetDevicePropertyW(a1, a2, propKey, propType, buf, size, reqSize, flags);
        if (reqSize && (propKey->fmtid.Data1 == MY_DEVPKEY_Device_FriendlyName.fmtid.Data1 && 
                  (propKey->pid == MY_DEVPKEY_Device_FriendlyName.pid || propKey->pid == MY_DEVPKEY_Device_DeviceDesc.pid))) {
            if (*reqSize < 512) *reqSize = 512;
        }
        if (reqSize && propKey->fmtid.Data1 == MY_DEVPKEY_Device_HardwareIds.fmtid.Data1 && propKey->pid == MY_DEVPKEY_Device_HardwareIds.pid) {
            if (*reqSize < 1024) *reqSize = 1024;
        }
        return res;
    }
    ...
    if (SUCCEEDED(hr) && pppSourceActivate && pcSourceActivate && *pcSourceActivate > 0) {
        ...
        for (UINT32 i = 0; i < count; i++) {
            if (ppDevices[i]) {
                ...
                // Get the original link to modify it dynamically
                wchar_t* origLink = NULL;
                UINT32 length = 0;
                if (SUCCEEDED(ppDevices[i]->GetAllocatedString(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_SYMBOLIC_LINK, &origLink, &length))) {
                    SpoofDevicePathW(origLink, length + 1);
                    HRESULT hrSym = ppDevices[i]->SetString(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_SYMBOLIC_LINK, origLink);
                    CoTaskMemFree(origLink);
                    LogA("[HOOK] Spoofed MF Symbolic Link result dynamically: 0x%lX", hrSym);
                }
            }
        }
    }
