...
// -----------------------------------------------------------------------------
// Dynamic path spoofer - replaces usb/vid/pid with pci/ven/dev in-place
// This is ONLY used for the MF symbolic link attribute (safe to spoof)
// -----------------------------------------------------------------------------
void SpoofDevicePathW(wchar_t* path, size_t maxLen) {
    if (!path || maxLen < 3) return;
    for (size_t i = 0; i < maxLen - 2; i++) {
        if (path[i] == L'\0') break;
        if ((path[i] == L'u' || path[i] == L'U') && (path[i+1] == L's' || path[i+1] == L'S') && (path[i+2] == L'b' || path[i+2] == L'B')) {
            path[i] = L'p'; path[i+1] = L'c'; path[i+2] = L'i';
        }
        else if ((path[i] == L'v' || path[i] == L'V') && (path[i+1] == L'i' || path[i+1] == L'I') && (path[i+2] == L'd' || path[i+2] == L'D')) {
            path[i] = L'v'; path[i+1] = L'e'; path[i+2] = L'n';
        }
        else if ((path[i] == L'p' || path[i] == L'P') && (path[i+1] == L'i' || path[i+1] == L'I') && (path[i+2] == L'd' || path[i+2] == L'D')) {
            path[i] = L'd'; path[i+1] = L'e'; path[i+2] = L'v';
        }
    }
}

// -----------------------------------------------------------------------------
// Registry Hooks - ONLY spoof FriendlyName/DeviceDesc
// -----------------------------------------------------------------------------
LSTATUS WINAPI Hook_RegQueryValueExW(HKEY hKey, LPCWSTR lpValueName, LPDWORD lpReserved, LPDWORD lpType, LPBYTE lpData, LPDWORD lpcbData) {
    // Save the INPUT buffer size before Windows overwrites it with the data size
    DWORD inputBufSize = (lpcbData ? *lpcbData : 0);
    ...
    if (status == ERROR_SUCCESS && lpValueName && lpData && lpcbData) {
        if (_wcsicmp(lpValueName, L"FriendlyName") == 0 || _wcsicmp(lpValueName, L"DeviceDesc") == 0) {
            ...
            // Use inputBufSize (the REAL buffer capacity), not *lpcbData (which is now the data size)
            if (inputBufSize >= bytesNeeded) {
                wcscpy((wchar_t*)lpData, fakeName);
                *lpcbData = (DWORD)bytesNeeded; // Update data size to match our fake name
    ...
    if (SUCCEEDED(hr) && pppSourceActivate && pcSourceActivate && *pcSourceActivate > 0) {
        ...
        for (UINT32 i = 0; i < count; i++) {
            if (ppDevices[i]) {
                ...
                // Spoof the symbolic link attribute to hide USB VID:PID
                // This is SAFE: ActivateObject uses its own internal path copy, not this attribute
                ...
                if (SUCCEEDED(ppDevices[i]->GetAllocatedString(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_SYMBOLIC_LINK, &origLink, &length))) {
                    ...
                    ppDevices[i]->SetString(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_SYMBOLIC_LINK, origLink);
                    LogA("[HOOK] Spoofed MF Symbolic Link dynamically");
...
void SetupHooks() {
    ...
    if (hSetupApi) {
        ...
        // NOT hooking SetupDiGetDeviceInterfaceDetail - Chromium needs real paths to stream video
    }
